...
_running_scans: dict[str, str] = {}  # run_id -> "RUNNING" | "COMPLETED" | "FAILED"
...
async def run_scan_async(
    ...
    scan_complete event containing the full results."""
    run_id = str(uuid.uuid4())
    _running_scans[run_id] = "RUNNING"
...
@router.get("/run/{run_id}", response_model=ScanRunResponse)
async def get_scan_run(run_id: str) -> ScanRunResponse:
    """Poll for a completed scan result by run_id."""
    result = _scan_store.get(run_id)
    if result is None:
        status = _running_scans.get(run_id)
        if status is None:
            raise HTTPException(status_code=404, detail="Run not found")
        raise HTTPException(status_code=202, detail=f"Scan {status}")
    return result
